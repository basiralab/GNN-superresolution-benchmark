...
def load_random_files(return_matrix=False, include_diagonal=False):
    lr_train1_data = np.genfromtxt("data/Random-CV/Fold1/lr_split_1.csv", delimiter=",", skip_header=1)
    hr_train1_data = np.genfromtxt("data/Random-CV/Fold1/hr_split_1.csv", delimiter=",", skip_header=1)
    lr_train2_data = np.genfromtxt("data/Random-CV/Fold2/lr_split_2.csv", delimiter=",", skip_header=1)
    hr_train2_data = np.genfromtxt("data/Random-CV/Fold2/hr_split_2.csv", delimiter=",", skip_header=1)
    lr_train3_data = np.genfromtxt("data/Random-CV/Fold3/lr_split_3.csv", delimiter=",", skip_header=1)
    hr_train3_data = np.genfromtxt("data/Random-CV/Fold3/hr_split_3.csv", delimiter=",", skip_header=1)
    ...
    last_20_lr_train1 = lr_train1_data[-20:]
    last_20_hr_train1 = hr_train1_data[-20:]
    ...
    if return_matrix:
        ...
        def create_matrices(lr_data, hr_data):
            lr_matrices = torch.tensor([MatrixVectorizer.anti_vectorize(sample, LR_MATRIX_SIZE, include_diagonal) 
                                        for sample in lr_data],
                                        dtype=torch.float32)
            hr_matrices = torch.tensor([MatrixVectorizer.anti_vectorize(sample, HR_MATRIX_SIZE, include_diagonal) 
